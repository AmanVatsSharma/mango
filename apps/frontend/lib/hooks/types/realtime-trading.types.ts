 ...
 *
 * ⚠️  NOTE: These types were defined locally and do NOT match
 * libs/shared/types/src/index.ts. OrderStatus, OrderType, OrderSide values
 * may differ. Do NOT use these for type comparisons with backend API responses.
 * Prefer importing from @mango/shared-types instead.
 *
 * Key mismatches vs shared canonical types:
 *   - OrderStatus: local defines 'EXECUTED'/'EXPIRED' (no SUBMITTED/PARTIAL_FILLED/FILLED);
 *                  canonical is PENDING/SUBMITTED/PARTIAL_FILLED/FILLED/CANCELLED/REJECTED
 *   - OrderType:   local uses 'STOP_LOSS_MARKET'; canonical uses 'STOP_LIMIT'
 *
...
// Canonical product types: MIS (intraday), CNC (equity delivery), NRML (carry for derivatives)
// Legacy aliases kept for compatibility with older payloads.
export type ProductType = 'MIS' | 'CNC' | 'NRML' | 'INTRADAY' | 'DELIVERY' | 'MARGIN'
