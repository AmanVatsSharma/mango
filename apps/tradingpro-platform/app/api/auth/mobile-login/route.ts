 ...
 *
 *   Flow on the RN side:
 *     1. User enters phone+password → POST /api/auth/mobile-login
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  // --- Branch 2.5: Simple registration (no phone, no email) → skip OTP entirely ---
  // These users have mPin set during registration; go straight to mPin verification.
  // OTP requires a phone number, which simple registration deliberately omits.
