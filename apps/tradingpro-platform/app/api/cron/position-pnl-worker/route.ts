...
import { positionPnLWorker, type ProcessPositionPnLInput } from "@/lib/services/position/PositionPnLWorker"
...
function normalizeLimit(value: string | null): number {
  ...
  return Math.min(2000, Math.max(1, Math.trunc(parsedValue)))
}

function normalizeUpdateThreshold(value: string | null): number {
  const parsedValue = parseFiniteCronQueryNumber(value)
  if (parsedValue === null) {
    return 1
  }
...
function normalizeDryRun(value: string | null): boolean {
  if (!value) {
    return false
  }
...
function normalizeOptionalBoundedInteger(value: string | null, min: number, max: number): number | null {
  const parsedValue = parseFiniteCronQueryNumber(value)
  if (parsedValue === null) {
    return null
  }
...
function resolveCronSecrets(): string[] {
  const secretCandidates = [process.env.POSITION_PNL_WORKER_SECRET, process.env.CRON_SECRET]
...
function normalizeRouteErrorMessage(value: unknown): string {
  if (typeof value !== "string") {
    return "Failed to run position PnL worker"
  }
...
export async function GET(req: Request) {
  ...
  try {
    ...
    // Master risk toggle: skip all auto-close enforcement if admin has disabled it.
    const { getRiskEnforcementSettings, isRiskEnabled } = await import("@/lib/services/risk/risk-enforcement-settings")
    const riskSettings = await getRiskEnforcementSettings({ maxAgeMs: 0 })
    if (!isRiskEnabled(riskSettings)) {
      console.log("⏸️ [CRON-POSITION-PNL] Skipped: risk auto-close is disabled (riskAutoCloseEnabled=false or circuit breaker active)")
    ...
    const updateThreshold = normalizeUpdateThreshold(searchParams.get("updateThreshold"))
    const dryRun = normalizeDryRun(searchParams.get("dryRun"))
    const forceRun = normalizeDryRun(searchParams.get("forceRun"))
    const intradayEodBackstopMode =
      normalizeDryRun(searchParams.get("intradayEodSquareOff")) ||
      normalizeDryRun(searchParams.get("eodSquareOff")) ||
      normalizeDryRun(searchParams.get("eod"))
    const intradayEodForceRun = intradayEodBackstopMode || normalizeDryRun(searchParams.get("intradayEodForceRun"))
    const intradayEodPreCloseBufferMinutes = normalizeOptionalBoundedInteger(
      searchParams.get("intradayEodPreCloseBufferMinutes"),
      1,
      120,
    )
    const intradayEodMaxAutoClosesPerTick = normalizeOptionalBoundedInteger(
      searchParams.get("intradayEodMaxAutoClosesPerTick"),
      0,
      5000,
    )

    const workerInput: ProcessPositionPnLInput = {
      limit,
      updateThreshold,
      dryRun,
    }
    if (forceRun || intradayEodBackstopMode) {
      workerInput.forceRun = true
    }
    if (intradayEodForceRun) {
      workerInput.intradayEodForceRun = true
    }
    if (intradayEodPreCloseBufferMinutes !== null) {
      workerInput.intradayEodPreCloseBufferMinutes = intradayEodPreCloseBufferMinutes
    }
    if (intradayEodMaxAutoClosesPerTick !== null) {
      workerInput.intradayEodMaxAutoClosesPerTick = intradayEodMaxAutoClosesPerTick
    }

    const result = await positionPnLWorker.processPositionPnL(workerInput)
    const autoCleanup = await runScheduledCleanupTick({ source: "cron_position_pnl_worker" })

    return NextResponse.json(
      {
        success: true,
        timestamp: new Date().toISOString(),
        mode: intradayEodBackstopMode ? "intraday_eod_backstop" : "standard",
        requested: {
          dryRun,
          forceRun: workerInput.forceRun === true,
          intradayEodForceRun: workerInput.intradayEodForceRun === true,
          intradayEodPreCloseBufferMinutes: workerInput.intradayEodPreCloseBufferMinutes ?? null,
          intradayEodMaxAutoClosesPerTick: workerInput.intradayEodMaxAutoClosesPerTick ?? null,
        },
        result,
        autoCleanup,
      },
