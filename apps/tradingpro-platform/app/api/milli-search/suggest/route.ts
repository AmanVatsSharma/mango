/**
 * File:        app/api/milli-search/suggest/route.ts
 * Module:      API · Search Proxy · Suggest
 * Purpose:     Server-side proxy for the marketdata suggest endpoint, adding the admin
 ...
 *
 * Exports:
 *   - GET(request) → NextResponse  — proxies to /api/search/suggest with auth + ?include=internal
 *
 * Depends on:
 ...
 *
 * Key invariants:
 *   - ltp_only=true is injected by default (only live instruments for search UX)
 *   - ?include=internal adds broker tokens when MARKETDATA_ADMIN_TOKEN is set
...
export async function GET(request: NextRequest) {
  ...
  // Forward client disconnect so upstream fetch is cancelled immediately
  request.signal.addEventListener('abort', () => abort.abort(), { once: true })

  try {
    const src = request.nextUrl
    const target = new URL(`${BASE}/api/search/suggest`)
