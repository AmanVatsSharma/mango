/**
 * @file components/marketing/stocktrade-home/stocktrade-sections.tsx
 * @module marketing/stocktrade-home
 * @description StockTrade homepage sections - hero, stats, highlights, payments, platforms, benefits, blog preview.
 *
 * Exports:
 *   - StockTradeHeroSection          - full-bleed desktop hero + stacked mobile layout
 *   - StockTradeStatsSection         - stat bar with CTAs
 *   - StockTradeHighlightsSection    - feature highlight cards
 *   - StockTradeCashSettlementSection - payment methods overview
 *   - StockTradePlatformsSection     - platform cards + APK/IPA download CTAs
 *   - StockTradeBenefitsAndMarginSection - benefits + margin explainer with image
 *   - StockTradeOpenAccountSection   - CTA banner
 *   - StockTradeBlogPreviewSection   - blog teaser cards
 *
 * Depends on:
 *   - @/Branding - BRAND_ASSETS, BRAND_IDENTITY, BRAND_MARKETING for all branding values
 *   - @/lib/marketing/stocktrade-homepage-content - copy/content config
 ...
 *
 * Read order:
 *   1. StockTradeHeroSection - entry point; full-bleed hero with responsive layout
 *   2. StockTradePlatformsSection - has the APK slug pattern to reference
 *
 * Author:      BharatERP
...
import { DEFAULT_HOME_PAGE_CONFIG } from "@/lib/marketing/stocktrade-homepage-content"
import { TickerTape } from "@/components/marketing/ticker-tape"

const routes = getUiRoutes()

const STOCKTRADE_HOMEPAGE_CONTENT = {
  hero: {
...
function PrimaryCta({ href, children }: { href: string; children: React.ReactNode }): React.JSX.Element {
  return (
    <Link
      href={href}
      className="stocktrade-cta-primary inline-flex items-center justify-center px-5 py-3 text-sm font-semibold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2"
    >
      {children}
    </Link>
  )
...
export function StockTradeHeroSection(): React.JSX.Element {
  const heroImage = (
    <Image
      ...
      <div className="relative z-10 mx-auto max-w-7xl w-full px-4 py-8 sm:px-6 sm:py-10 lg:px-8 lg:py-24 order-1 lg:order-none">
        <div className="max-w-2xl">
          ...
          >
            ...
            >
              {STOCKTRADE_HOMEPAGE_CONTENT.hero.headline}
            </motion.h1>
            ...
            >
              {STOCKTRADE_HOMEPAGE_CONTENT.hero.productTabs.map((tab) => (
                <span
                  key={tab}
            ...
            >
              {STOCKTRADE_HOMEPAGE_CONTENT.hero.subheadline}
            </motion.p>
            ...
            >
              <Link
                href={STOCKTRADE_HOMEPAGE_CONTENT.hero.ctas.primaryHref}
                className="stocktrade-cta-primary inline-flex items-center justify-center px-6 py-3.5 text-sm font-semibold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 transition-transform hover:scale-105"
              >
                {STOCKTRADE_HOMEPAGE_CONTENT.hero.ctas.primaryLabel} <ChevronRight className="ml-2 w-4 h-4" />
              </Link>
              <Link
                href={STOCKTRADE_HOMEPAGE_CONTENT.hero.ctas.secondaryHref}
              ...
              >
                {STOCKTRADE_HOMEPAGE_CONTENT.hero.ctas.secondaryLabel}
              </Link>
            </motion.div>
          </motion.div>

          {/* Ticker tape below headline, above quick links */}
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            transition={{ delay: 0.9, duration: 0.5 }}
          >
            <TickerTape symbols={DEFAULT_HOME_PAGE_CONFIG.tickerSymbols} />
          </motion.div>
          ...
          >
            {BRAND_MARKETING.homepage.heroQuickLinks.slice(0, 2).map((quickLink) => (
              <Link
                ...
                className="stocktrade-card-premium rounded-xl border border-white/20 bg-white/10 backdrop-blur-md p-4 transition-all hover:bg-white/20"
              >
...
export function StockTradeStatsSection(): React.JSX.Element {
  return (
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        >
          <div className="flex items-center space-x-4">
            ...
            <div>
              <p className="text-3xl font-extrabold text-primary">{STOCKTRADE_HOMEPAGE_CONTENT.stats.value}</p>
              <p className="mt-1 text-sm font-bold uppercase tracking-wide text-slate-500">{STOCKTRADE_HOMEPAGE_CONTENT.stats.label}</p>
            </div>
          </div>
          <div className="lg:col-span-2">
            <div className="flex flex-wrap gap-4 lg:justify-end">
              <SecondaryCta href={STOCKTRADE_HOMEPAGE_CONTENT.stats.ctas.leftHref}>
                {STOCKTRADE_HOMEPAGE_CONTENT.stats.ctas.leftLabel}
              </SecondaryCta>
              <PrimaryCta href={STOCKTRADE_HOMEPAGE_CONTENT.stats.ctas.rightHref}>
                {STOCKTRADE_HOMEPAGE_CONTENT.stats.ctas.rightLabel}
              </PrimaryCta>
            </div>
          </div>
        </motion.div>
...
export function StockTradeHighlightsSection(): React.JSX.Element {
  return (
    <section>
      <div className="mx-auto max-w-7xl px-4 py-4 sm:px-6 lg:px-8">
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {STOCKTRADE_HOMEPAGE_CONTENT.highlights.map((label) => (
            <div key={label} className="stocktrade-card-premium rounded-xl border bg-white p-5">
...
export function StockTradeCashSettlementSection(): React.JSX.Element {
        ...
        <div className="grid gap-8 lg:grid-cols-2 lg:items-center">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {paymentItems.map((item) => (
              <Link key={item.href} href={item.href} className="stocktrade-card-premium rounded-xl border bg-white p-5 hover:bg-slate-50">
...
export function StockTradePlatformsSection(): React.JSX.Element {
    ...
    <section id="platforms" className="bg-slate-50/50 py-16">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-6">
            <div className="flex flex-wrap items-center justify-center gap-3">
              <a
                href={androidApkPath}
                download
                className="stocktrade-cta-primary inline-flex items-center justify-center px-6 py-3 text-sm font-semibold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2"
        ...
        <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {STOCKTRADE_HOMEPAGE_CONTENT.platforms.map((platform, i) => (
            <motion.div
              key={platform.label}
            ...
            >
              <Link
                href={platform.href}
                className="group block stocktrade-card-premium rounded-2xl border border-slate-200 bg-white p-8 text-center shadow-sm hover:shadow-xl transition-all duration-300"
              >
...
export function StockTradeBenefitsAndMarginSection(): React.JSX.Element {
            ...
            <div className="mt-12 grid gap-4 sm:grid-cols-2">
              {cards.map((card, i) => (
                <motion.div
                  ...
                  className="stocktrade-card-premium rounded-xl border border-slate-200/60 bg-slate-50/50 p-5 hover:bg-white hover:shadow-lg transition-all"
                >
...
export function StockTradeOpenAccountSection(): React.JSX.Element {
  return (
        ...
        >
          ...
          <div className="flex gap-4 lg:justify-end relative z-10">
            <Link
              ...
              className="stocktrade-cta-primary inline-flex items-center justify-center px-8 py-4 text-base font-bold shadow-lg shadow-primary/20 transition-transform hover:scale-105 hover:shadow-xl hover:shadow-primary/30"
            >
...
export function StockTradeBlogPreviewSection(): React.JSX.Element {
  return (
    <section id="news">
      <div className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {STOCKTRADE_HOMEPAGE_CONTENT.blogTitles.map((title) => (
            <Link key={title} href={routes.marketing.blog} className="group stocktrade-card-premium overflow-hidden rounded-xl border bg-white hover:bg-slate-50">
