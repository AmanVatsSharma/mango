...
- **MARKET**: require server WS quote within a configured freshness window (default `<=60s`) at placement and execution time.
- **MARKET fallback policy**: no stale/snapshot fallback; if fresh quote is missing, cancel with persisted exchange-rejected reason.
- **LIMIT**: execute using order-locked limit price.
...
- `tests/api/trading-orders-route.test.ts`
- `tests/api/trading-orders-mutations-route.test.ts`
- `tests/api/trading-orders-status-route.test.ts`
- `tests/api/trading-orders-list-route.test.ts`

**Change-log:**
- 2026-02-27: `/api/trading/orders` identity guard now tolerates futures-style `strikePrice: 0` sentinels when `optionType` is absent, while still enforcing positive strike validation for option identities.
...
- 2026-02-24: Added bounded server quote warmup (`waitForFreshQuote`) in MARKET placement/worker paths to avoid cold-cache false stale rejections, prioritized `Stock.token` resolution for worker subscriptions/execution, and attached server-feed health context to exchange-reject diagnostics.
- 2026-02-24: Enforced live-price integrity policy for MARKET orders across form/service/worker paths: server quote within the configured freshness window (originally `<=5s`, now default `<=60s`) is mandatory; stale/missing quotes produce persisted `CANCELLED` orders with `failureCode`/`failureReason` exposed by list/status APIs and UI.
- 2026-02-24: Order execution now persists segregated position lots by contract + canonical product mode and performs FIFO opposite-side lot offset before opening remainders, preventing equity/F&O/commodity and MIS/DELIVERY cross-mixing in positions.
...
- 2026-02-16: `/api/cron/order-worker` plain-object authorization parsing now matches header keys case-insensitively (e.g., `aUtHoRiZaTiOn`) for broader adapter compatibility.
- 2026-02-16: `/api/cron/order-worker` authorization header normalization now supports array-valued header carriers (e.g., `authorization: ["Bearer ..."]`) commonly emitted by proxy adapters.
- 2026-02-16: `/api/cron/order-worker` authorization parsing now also supports nested plain-object header wrappers (`headers.headers.authorization`) used by some adapter layers.
- 2026-02-16: `/api/cron/order-worker` authorization parsing now also supports iterable header-entry wrappers (e.g., `[["authorization","Bearer ..."]]`) when adapters expose tuple-based header maps.
...
- 2026-02-16: `/api/cron/order-worker` now accepts either `ORDER_WORKER_SECRET` or `CRON_SECRET` when both are configured, improving secret-rotation compatibility for scheduled execution.
...
- 2026-02-16: `/api/cron/order-worker` cron secret parsing now unwraps quoted secret tokens (`"secret"` / `'secret'`) before auth comparison for compatibility with quoted env serialization styles.
...
- 2026-02-16: `/api/cron/order-worker` query parsing now also accepts URL-object wrappers exposing `pathname/search` (including callable fields), preserving param extraction when `href` is unavailable.
...
- 2026-02-16: `/api/cron/order-worker` 500 error payloads now normalize worker error messages (trim + whitespace collapse + 256-char bound) before response serialization.
- 2026-02-16: `OrderExecutionWorker.processPendingOrders` now normalizes malformed runtime payloads (`limit`, `maxAgeMs`, non-object input) to safe defaults/clamped values before pending-order batch queries.
...
- 2026-02-16: `OrderExecutionWorker.processOrderById` now trims/validates runtime `orderId` inputs (rejecting blank/oversized values) before advisory-lock and DB query execution.
- 2026-02-15: Trading order mutation routes now enforce session ownership boundaries and return normalized access errors.
- 2026-02-15: Trading order routes now use a shared error resolver helper to keep catch-block status/message mapping consistent.
- 2026-02-15: Shared trading error resolver now normalizes invalid status/message payloads before sending order route responses.
- 2026-02-15: Shared trading error resolver now maps Prisma error signatures (`P2025`, `P2002`, validation) to stable HTTP statuses.
- 2026-02-15: Shared trading error resolver now honors valid upstream `status`/`statusCode` values on thrown errors for order route responses.
- 2026-02-15: Shared trading error resolver now accepts numeric-string upstream statuses (e.g. `"429"`) while rejecting malformed values.
...
- 2026-02-15: `/api/trading/orders/status` now uses shared auth/ownership/error resolver guards with defensive missing-order handling after ownership checks.
- 2026-02-15: `/api/trading/orders/list` now uses shared auth/error resolver guards for consistent unauthorized/forbidden/error mapping.
- 2026-02-15: `/api/trading/orders/status` now emits standardized API telemetry (`trading_order_status`) while retaining shared guard/error behavior.
