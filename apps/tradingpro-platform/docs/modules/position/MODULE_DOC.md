...
- `lib/services/position/position-number-utils.ts` — shared strict finite-number parsing for position services/workers
...
- `POSITION_PNL_WORKER_LOCK_TTL_MS` (default `120000`) global overlap lock lease TTL
...
- `PositionPnLWorker` now closes intraday MIS/INTRADAY positions near close (segment-aware pre-close window).
- Product-type detection uses executed position orders (entry-side preferred, latest executed fallback).
- Segment close references:
  - NSE/NFO family: `15:30 IST`
  - MCX family: `23:55 IST`
- Window rule: `closeTime - preCloseBufferMinutes <= now <= closeTime`.
- Per-day/per-segment marker keys in `SystemSettings` prevent duplicate reruns:
  - `positions_intraday_eod_squareoff_<segment>_<yyyy-mm-dd>`
- Heartbeat now includes:
  - `intradayEodCandidates`, `intradayEodClosed`, `intradayEodSkipped`, `intradayEodMarkersWritten`

**Cron EOD backstop trigger:**
- `GET /api/cron/position-pnl-worker?eod=1`
- Dry-run validation: `GET /api/cron/position-pnl-worker?eod=1&dryRun=1`
- Optional tuning:
...
- `tests/position/position-pnl-worker-overlap.test.ts`
- `tests/position/position-pnl-worker-global-lock.test.ts`
- `tests/api/cron-position-pnl-worker-route.test.ts`
- `tests/position/position-management-atomic-close.test.ts`
- `tests/api/trading-positions-route.test.ts`
- `tests/api/trading-positions-list-route.test.ts`

**Change-log:**
- 2026-02-24: `PositionPnLWorker` now prefers persisted `Stock.token` over instrument parsing, warms resolved quote subscriptions with bounded `waitForFreshQuote(...)` before each MTM pass, and keeps explicit live-quote/fallback hit tracking.
- 2026-02-24: Positions now persist as segregated execution lots with explicit identity metadata and FIFO opposite-side offseting scoped to contract + product type, with `/api/trading/positions/list` exposing identity/product payload fields used by UI grouping to avoid underlying-prefix collisions.
- 2026-02-23: Dashboard open-position MTM resolution now follows hybrid-live rendering (live quote-derived MTM first, server snapshot fallback/sync), admin positions list now overlays fresh Redis PnL snapshots for open rows, and admin positions UI patches `positions_pnl_updated` events directly for smoother live MTM.
- 2026-02-23: `PositionPnLWorker` heartbeat now includes token/quote coverage telemetry (`positionsWithResolvedToken`, `positionsWithoutResolvedToken`, `positionTokensResolved`, `positionsWithLiveQuote`, `positionsWithoutLiveQuote`, `quoteHitRate`) to verify server-side quote quality during live MTM computation.
- 2026-02-23: Positions MTM header now shows **Open MTM** (open-only) + **Booked (Today)** (IST day) and `/api/trading/positions/list` now returns open positions + **today-closed only** (bounded list) using new `Position.closedAt`; server mode now keeps worker snapshot PnL authoritative when fresh (live quotes are fallback when stale/unhealthy).
- 2026-02-21: Dashboard/positions PnL rendering now follows hybrid-smart semantics (live quote preferred for smooth UI, server snapshot fallback/sync) through a shared resolver path; `/api/trading/positions/list` now includes per-position `pnlUpdatedAtMs` and meta `pnlMaxAgeMs` for deterministic freshness checks.
...
- 2026-02-21: Added segment-aware intraday EOD square-off in `PositionPnLWorker` with configurable pre-close buffer, per-day/per-segment marker idempotency, and heartbeat counters for candidate/close/skip visibility.
- 2026-02-21: Added shared `position-product-type-utils` intraday resolver (entry-side executed order preferred, latest executed fallback) and wired `PositionManagementService` close-path product-type resolution to it.
- 2026-02-21: `/api/cron/position-pnl-worker` now supports explicit EOD backstop params (`eod|intradayEodSquareOff`, `intradayEodForceRun`, buffer/max-close overrides) while preserving auth + `dryRun`.
- 2026-02-21: `/api/trading/positions` PATCH now accepts omitted `tradingAccountId` by deriving account ownership from position context, while still rejecting explicit mismatch/non-string account payloads.
- 2026-02-21: `PositionManagementService.updatePosition` now enforces directional SL/TP bounds against a live/fallback reference price (`LONG: SL < ref < TP`, `SHORT: TP < ref < SL`) before DB update.
- 2026-02-21: `/api/trading/positions/list` now includes richer F&O metadata at top-level (`segment`, `instrumentId`, `strikePrice`, `optionType`, `expiry`, `token`) and keeps nested `stock` shape compatibility.
- 2026-02-21: Prisma realtime middleware now skips `position_updated` emits for PnL-only writes (`unrealizedPnL/dayPnL`) to reduce server-mode refetch churn while preserving lifecycle sync events.
...
- 2026-02-15: Position close flow now performs fund settlement through tx-scoped methods in the same transaction boundary.
- 2026-02-15: Added in-process overlap guard for `PositionPnLWorker` to prevent duplicate concurrent runs.
- 2026-02-15: Added global DB-backed lock guard for `PositionPnLWorker`, returning `reason=locked` when overlap is detected.
- 2026-02-16: Position PnL cron route now normalizes malformed query/URL inputs, clamps limit/updateThreshold safely, and accepts broader `dryRun` truthy variants.
...
- 2026-02-16: Position PnL cron route plain-object authorization parsing now matches header keys case-insensitively (e.g., `aUtHoRiZaTiOn`) for broader adapter compatibility.
- 2026-02-16: Position PnL cron route authorization header normalization now supports array-valued header carriers (e.g., `authorization: ["Bearer ..."]`) commonly emitted by proxy adapters.
- 2026-02-16: Position PnL cron route authorization parsing now also supports nested plain-object header wrappers (`headers.headers.authorization`) used by some adapter layers.
- 2026-02-16: Position PnL cron route authorization parsing now also supports iterable header-entry wrappers (e.g., `[["authorization","Bearer ..."]]`) when adapters expose tuple-based header maps.
...
- 2026-02-16: Position PnL cron route now accepts either `POSITION_PNL_WORKER_SECRET` or `CRON_SECRET` when both are configured, improving secret-rotation compatibility for scheduled execution.
...
- 2026-02-16: Position PnL cron route secret parsing now unwraps quoted secret tokens (`"secret"` / `'secret'`) before auth comparison for compatibility with quoted env serialization styles.
...
- 2026-02-16: Position PnL cron route query parsing now also accepts URL-object wrappers exposing `pathname/search` (including callable fields), preserving param extraction when `href` is unavailable.
...
- 2026-02-16: Position PnL cron route 500 payloads now normalize worker error messages (trim + whitespace collapse + 256-char bound) before API response serialization.
- 2026-02-16: `PositionPnLWorker.processPositionPnL` now normalizes malformed runtime payloads (`limit`, `updateThreshold`, `dryRun`, non-object input) so direct/non-route worker calls still run with deterministic safe defaults.
...
- 2026-02-15: Position mutation route catches now use a shared trading error resolver for consistent response status/message mapping.
...
- 2026-02-15: Shared trading error resolver now maps Prisma error signatures (`P2025`, `P2002`, validation) to stable HTTP statuses.
- 2026-02-15: Shared trading error resolver now honors valid upstream `status`/`statusCode` values on thrown errors for position route responses.
...
- 2026-02-15: `/api/trading/positions/list` now uses shared auth/error resolver guards for consistent unauthorized/forbidden/error status mapping.
